...
class BoardOptionsManager:
    ...
    def only_value_in_square(self, row, col, value):
        square_start_row = row - (row % 3)
        square_start_col = col - (col % 3)
        return len([(k, p) for p in range(3) for k in range(3) if
                    value in self.options[square_start_row + p][square_start_col + k]]) == 1
    ...
    def print_options(self):
        ...
        for i, row in enumerate(self.options):
            x.add_row([str(i + 1), *[','.join([str(option) for option in options]) for options in row]])
        print(x)
